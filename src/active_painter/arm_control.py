...
def ik_pose_for_canvas_point(x: float, z: float, depth: float, elbow_up: bool = True) -> ArmPose:
    ...
    cos_elbow = np.clip((dist2 - l1 * l1 - l2 * l2) / (2.0 * l1 * l2), -1.0, 1.0)
    ...
    return ArmPose(
        ...
        pitch=float(np.clip(np.rad2deg(pitch), -90.0, 90.0)),
        roll=0.0,
        elbow=float(np.clip(np.rad2deg(elbow), 0.0, 150.0)),
    )
...
def scripted_contact_pressure(t: float) -> float:
    ...
    if u < 0.84:
        p = (u - 0.26) / 0.58
        pressure = 0.10 + 0.34 * np.sin(np.pi * p) ** 2 + 0.08 * np.sin(5.0 * np.pi * p + index)
        return float(np.clip(pressure, 0.03, 0.52))
    p = (u - 0.84) / 0.16
