...
The `provisional-compact-dual-imx296-v1` acquisition contract assigns two
selected-but-not-yet-purchased Raspberry Pi Global Shutter Cameras (IMX296)
to the separate left/right oblique views. The provisional 4 mm CS lenses,
1456 x 1088 acquisition, 60 Hz rate, timing, and nominal intrinsics live in
the XML but remain hidden calibration parameters, not agent observations.
The contract declares independent 512 x 512 global and 256 x 256 foveal
products. The simulator now retains the native frame, emits
...
`ContactState` contains exact on-canvas status, bushing deflection, normal
force, pressure, brush width, rigid contact point, aggregate tangential force,
pull alignment, sticking/release state, slip fraction, and bristle deflection.
The added anisotropic fields are generated by the provisional process from
exact kinematics, contact, and canvas tooth. They are simulator-only labels;
there is no likelihood that observes them directly.
...
distance, bristle gains, streak phases, edge-wobble phases, aggregate
tangential deflection/stick state, and RNG state. These are hidden causes of
visible marks.
...
contamination, bristle state, tangential force, sticking, and tip lag remain
latent and must ultimately be inferred from camera, encoder, current, and any
future force/contact observations. Exact anisotropic-process telemetry is a
training/evaluation target, not evidence available to painting inference.
...
native-fallback leakage, and reuse of the exact reduced brush-contact equations
remain. Independent initialization is necessary but does not turn a process
simulator into a learned uncertain generative model.
...
- exact contact state;
- exact aggregate brush deflection, stick/slip, pull alignment, tangential
  load, and process painting point;
- exact coverage;
- true link velocity;
